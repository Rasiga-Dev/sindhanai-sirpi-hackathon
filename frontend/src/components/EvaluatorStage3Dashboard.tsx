...
const EvaluatorStage3Dashboard = () => {
    ...
    const fetchProjects = async () => {
        try {
            ...
            const response = await axios.get(`${API_BASE}/api/evaluator/getAcceptedEvaluatedProjects`);
    ...
    const handleSubmit = async (schoolId, projectId, status) => {
        try {
            setLoading(true);
            const response = await axios.post(`${API_BASE}/api/evaluator/update-winner`, {
                schoolId,
                projectId,
                status
            });
