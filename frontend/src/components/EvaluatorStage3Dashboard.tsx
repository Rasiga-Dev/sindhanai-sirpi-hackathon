...
const EvaluatorStage3Dashboard = () => {
    ...
    const fetchProjects = async () => {
        try {
            ...
            const response = await axios.get(`${VITE_BASE_URL}/evaluator/getAcceptedEvaluatedProjects`);
    ...
    const handleSubmit = async (schoolId, projectId, status) => {
        try {
            setLoading(true);
            const response = await axios.post(`${VITE_BASE_URL}/evaluator/update-winner`, {
                schoolId,
                projectId,
                status
            });
