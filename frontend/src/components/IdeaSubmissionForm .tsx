...
//         `${VITE_BASE_URL}/schools/drafts/${udiseCode}`,
//         {
//           headers: { Authorization: `Bearer ${token}` }
//         }
//       );
...
//       const response = await axios.get(`${VITE_BASE_URL}/school/get-registered-guide-teachers`, {
//         headers: {
...
//       await axios.post(
//         `${VITE_BASE_URL}/schools/submit-idea`,
//         formData,
...
//       const token = localStorage.getItem('schoolToken');
//       await axios.delete(
//         `${VITE_BASE_URL}/schools/drafts/${parsedSchoolDetails.udiseCode}`,
//         { headers: { Authorization: `Bearer ${token}` } }
//       );
...
const IdeaSubmissionForm: React.FC = () => {
 ...
 const handleFinalSubmission = async ({ transactionId }: { transactionId: string }) => {
  setIsSubmitting(true);
  try {
    const schoolDetails = localStorage.getItem('schoolDetails');
    if (!schoolDetails) throw new Error('School details not found in local storage');
    ...
    // Move to Thank You step (index 5)
    setCurrentStep(5);
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      ...
      case 3:
        return (
          <QRCodePaymentForm
            onBack={handleBack}
            onNext={({ transactionId }) => {
              setPendingTransactionId(transactionId); // save for confirmation / final submit
              setCurrentStep(4); // go to Confirmation step (index 4)
              saveDraft({
                transactionId,
                currentStep: 4
              });
      ...
      case 4:
        // Confirmation step - show details & final confirm button
        return (
          <div className="max-w-xl mx-auto bg-white p-6 rounded-md shadow">
            ...
            <div className="mb-4 text-sm text-gray-700">
              {/* <p>
                ...
                <span className="font-mono bg-gray-100 px-2 py-1 rounded">{pendingTransactionId || '—'}</span>
              </p> */}
              <p className="mt-3">
                Once you confirm, you will not be able to edit all details.
              </p>
            </div>

            <div className="flex justify-between">
              <button
                onClick={() => setCurrentStep(3)} // go back to QR payment to edit if needed
              ...
              <button
                onClick={() => handleFinalSubmission({ transactionId: pendingTransactionId })}
