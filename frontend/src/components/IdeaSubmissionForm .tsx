...
const IdeaSubmissionForm: React.FC = () => {
  ...
  const loadDraft = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE}/api/schools/drafts/${udiseCode}`,
        {
          headers: { Authorization: `Bearer ${token}` }
        }
      );
  ...
  const fetchGuideTeachers = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE}/api/school/get-registered-guide-teachers`, {
        headers: {
  ...
  const handleFinalSubmission = async ({ transactionId }) => {
    ...
    try {
      ...
      await axios.post(
        `${API_BASE}/api/schools/submit-idea`,
        formData,
      ...
      const token = localStorage.getItem('schoolToken');
      await axios.delete(
        `${API_BASE}/api/schools/drafts/${parsedSchoolDetails.udiseCode}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
