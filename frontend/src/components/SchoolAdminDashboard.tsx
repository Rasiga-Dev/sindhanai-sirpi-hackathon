...
const [showBMC, setShowBMC] = useState(false);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const schoolRes = await axios.get(`${VITE_BASE_URL}/schools/${id}`);
        setSchool(schoolRes.data);

        const dashboardRes = await axios.get(`${VITE_BASE_URL}/admin/summary/${id}`);
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
              ...
              <div className="md:col-span-9 space-y-6">
                <div className="flex gap-4 border-b border-gray-200 mb-4">
                  {['guide', 'student', 'pending', 'completed'].map((tab) => (
                    ...
                    >
                      ...
                      {tab === 'pending' && 'Pending Projects List'}
                ...
                <div className="p-6 rounded-xl bg-white shadow-sm border">
                  ...
                  {activeTab === 'pending' && (
                    <div>
                      <h2 className="text-lg font-semibold mb-4">Pending Projects List</h2>
                      {dashboardData?.pendingProjectsList?.length > 0 ? (
                        <div className="overflow-x-auto">
                          <table className="min-w-full border text-sm text-left">
                            <thead className="bg-yellow-100 text-yellow-800">
                              <tr>
                                ...
                                <th className="border px-4 py-2">Evaluation Status</th>
                              </tr>
                            </thead>
                            <tbody>
                              {dashboardData.pendingProjectsList.map((project, i) => (
                                <tr key={i} className="hover:bg-yellow-50">
                                  <td className="border px-4 py-2">{i + 1}</td>
                                  <td className="border px-4 py-2">{project.projectDetails?.title}</td>
                                  <td className="border px-4 py-2">{project.paymentStatus}</td>
                                  <td className="border px-4 py-2">{project.evaluationStatus || 'Not Evaluated'}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                          ...
                          <table className="min-w-full border border-gray-300 rounded text-sm">
                            <thead className="bg-green-100">
                              <tr>
                                ...
                                <th className="border px-4 py-2 text-left">Evaluator</th>
                                <th className="border px-4 py-2 text-left">Evaluation Scores</th>
                            ...
                            <tbody>
                              {dashboardData.completedProjectsList.map((project, index) => (
                                <tr key={index} className="bg-white hover:bg-gray-50">
                                  ...
                                  <td className="border px-4 py-2 capitalize">{project.paymentStatus}</td>
                                  ...
                                  <td className="border px-4 py-2">
                                    {project.evaluatedBy?.name || 'N/A'}<br />
                                    <span className="text-xs text-gray-500">{project.evaluatedBy?.email}</span>
                                  </td>
                                  <td className="border px-4 py-2">
                                    {project.evaluationScores?.length > 0 ? (
                                      <ul className="list-disc pl-4">
                                        {project.evaluationScores.map((score, i) => (
                                          <li key={i}>
                                            {score.evaluatorName}: <strong>{score.score}</strong> ({score.status})
                                          </li>
                                        ))}
                                      </ul>
                                    ) : 'N/A'}
                                  ...
                                  <td className="border px-4 py-2">
                                    {project.paymentScreenshot?.base64 ? (
                                      <img
                                        ...
                                        className="w-24 h-auto rounded shadow cursor-pointer hover:scale-105 transition-transform"
                                        onClick={() => {
                                          setPreviewImage(`data:${project.paymentScreenshot.contentType};base64,${project.paymentScreenshot.base64}`);
                                          setShowModal(true);
                                        }}
                                      />
                                    ) : 'N/A'}
