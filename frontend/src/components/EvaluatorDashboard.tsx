...
export default function EvaluatorDashboard() {
  ...
  const fetchL1Projects = async () => {
    ...
    try {
      const token = localStorage.getItem('evaluatorToken');
      const response = await axios.get(`${VITE_BASE_URL}/evaluator/assigned-projects`, {
  ...
  const fetchL1ProjectsList = async () => {
    ...
    try {
      const token = localStorage.getItem('evaluatorToken');
      const response = await axios.get(`${VITE_BASE_URL}/evaluator/level-1-list`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchL2Projects = async () => {
    try {
      const token = localStorage.getItem('evaluatorToken');
      const res = await axios.get(`${VITE_BASE_URL}/evaluator/projects-to-evaluate`, {
        headers: { Authorization: `Bearer ${token}` },
  ...
  const fetchL2ProjectsList = async () => {
    try {
      const token = localStorage.getItem('evaluatorToken');
      const res = await axios.get(`${VITE_BASE_URL}/evaluator/level-2-list`, {
        headers: { Authorization: `Bearer ${token}` }
      });
