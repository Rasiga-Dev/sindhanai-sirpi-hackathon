...
const JullyMark = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`${VITE_BASE_URL}/admin/jully-marks`)
            .then((res) => {
                ...
                fetched.forEach((item) => {
                    if (item.jullyMarks) {
                        saved[item.submissionId] = {
                            jury1: item.jullyMarks.jury1,
                            jury2: item.jullyMarks.jury2,
                            jury3: item.jullyMarks.jury3,
        ...
        axios
            .get(`${VITE_BASE_URL}/jully/latest`)
            .then((res) => {
                if (res.data) {
                    setPdfURL(`${VITE_BASE_URL}/jully/pdf/${res.data.filename}`);
    ...
    const calculateAverage = (markObj) => {
        const { jury1 = '', jury2 = '', jury3 = '', bannel = '' } = markObj || {};
        const nums = [jury1, jury2, jury3].map(n => Number(n));
    ...
    const handleExportPDF = () => {
        const doc = new jsPDF();
        const tableColumn = ['S.No', 'Project Name', 'Jury Mark 1', 'Jury Mark 2', 'Jury Mark 3'];
        const tableRows = [];
    ...
    const handleSave = async (item) => {
        ...
        const payload = {
            ...
            jury1: Number(markObj.jury1),
            jury2: Number(markObj.jury2),
            jury3: Number(markObj.jury3),
            bannel: Number(markObj.bannel),
            average: Number(calculateAverage(markObj)),
        };

        try {
            await axios.post(`${VITE_BASE_URL}/admin/jully-marks/save`, payload);
    ...
    const handleFileUpload = async (e) => {
        ...
        try {
            const response = await axios.post(`${VITE_BASE_URL}/jully/upload`, formData, {
                headers: {
            ...
            const previewURL = `${VITE_BASE_URL}/jully/pdf/${uploadedFilename}`;
                                ...
                                {filteredProjects.map((item, idx) => {
                                    ...
                                    return (
                                        <tr key={item.submissionId} className="hover:bg-gray-50">
                                            ...
                                            <td className="border px-2 py-2">{item.district}</td>
                                            {['jury1', 'jury2', 'jury3', 'bannel'].map((field) => (
                                            ...
                                            <td className="border px-2 py-2">
                                                <button
                                                    ...
                                                    disabled={
                                                        !markObj.jury1 || markObj.jury1 <= 0 ||
                                                        !markObj.jury2 || markObj.jury2 <= 0 ||
                                                        !markObj.jury3 || markObj.jury3 <= 0
                                                    }
                                                    className={`flex items-center font-medium ${(!markObj.jury1 || markObj.jury1 <= 0 ||
                                                            !markObj.jury2 || markObj.jury2 <= 0 ||
                                                            !markObj.jury3 || markObj.jury3 <= 0)
