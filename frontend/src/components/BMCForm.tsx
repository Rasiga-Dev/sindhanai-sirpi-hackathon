...
import { toast, ToastContainer } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import VITE_BASE_URL from '../config/api'; // keep if you use it later; otherwise remove
...
const BMCForm: React.FC<BMCFormProps> = ({ onNext, onBack, initialData }) => {
  const [formData, setFormData] = useState<Record<string, string>>(
    FIELDS.reduce((acc, f) => {
      acc[f.name] = (initialData && initialData[f.name]) || '';
      return acc;
  ...
  useEffect(() => {
    // compute validity whenever formData changes
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload = FIELDS.reduce((acc, f) => {
      acc[f.name] = (formData[f.name] || '').trim();
      return acc;
    }, {} as Record<string, string>);

    toast.success('BMC details saved successfully!');
    setTimeout(() => onNext(payload), 800); // small delay so user sees toast
  };
    ...
    <>
      ...
      <div className="mb-8">
        <iframe
          width="100%"
          height="335"
          src="https://www.youtube.com/embed/Yoyda1t7nT0?si=RIrwxhHSwY82sNZ4"
        ...
        <div className="flex justify-between mt-4">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-md ${
              isFormValid ? 'bg-red-800 text-white hover:bg-red-900' : 'bg-gray-300 text-gray-600 cursor-not-allowed'
            }`}
          >
            Next
          </button>
